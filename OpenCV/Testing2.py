...
def identifyObject (image_data,data_source):
	...
	lower = float(image_data[2])
	upper = float(image_data[3])
	matches = []
	for d in data:
		id_num = d[0]
		...
		lower_pct = abs((lower - lower_src)/lower_src)
		upper_pct = abs((upper - upper_src)/upper_src)
		pct_dif_avg = (convex_ratio_pct + area_ratio_pct + lower_pct + upper_pct ) / 4
